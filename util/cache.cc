...
struct LRUHandle { //双向链表节点
  ...
  uint32_t refs;      // 引用计数 References, including cache reference, if present.
  uint32_t hash;      // Hash of key(); used for fast sharding and comparisons
  char key_data[1];   // key指针 Beginning of key
...
class HandleTable {
 public:
  ...
  LRUHandle* Insert(LRUHandle* h) { // 插入节点,存在时替换，返回旧的
    ...
    LRUHandle* old = *ptr; //old==NULL表示没找到
    h->next_hash = (old == NULL ? NULL : old->next_hash); //如果存在则替换，否则插入到最后
    *ptr = h;
  ...
  LRUHandle* Remove(const Slice& key, uint32_t hash) {//如果存在key节点，则从哈希表中删除
 ...
 private:
  ...
  LRUHandle** FindPointer(const Slice& key, uint32_t hash) { //如果找到就返回指向命中节点的指针 ,否则返回对应double linked list的结尾
  ...
  void Resize() { //以两倍的大小重新设置哈希表长度
    ...
    uint32_t count = 0;
    for (uint32_t i = 0; i < length_; i++) { //重新哈希，一次全部执行完毕
...
Cache::Handle* LRUCache::Insert(
  ...
  if (capacity_ > 0) {
    ...
    e->in_cache = true;
    LRU_Append(&in_use_, e); // 加入LRU队列
    usage_ += charge;
    FinishErase(table_.Insert(e)); // 如果存在旧的相同的key，则插入新的，返回旧的
...
class ShardedLRUCache : public Cache {
 ...
 public:
  ...
  virtual void* Value(Handle* handle) { //返回对应值
