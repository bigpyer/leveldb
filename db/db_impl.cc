...
Status DBImpl::Write(const WriteOptions &options, WriteBatch *my_batch)
{
  ...
  { // NULL batch is for compactions
    ...
    // into mem_.
    {
      // 写日志和写memtable时可以释放锁，让别的write进入deque，尽可能减少互斥时间
      mutex_.Unlock();
      //append wal record
      // 写wal日志
...
Status DBImpl::MakeRoomForWrite(bool force)
{
  ...
  while (true)
  {
        ...
        versions_->NumLevelFiles(0) >= config::kL0_SlowdownWritesTrigger)
    {
      ...
      // sleep 1秒
      // 如果此时level 0中的文件个数已经大于等于kL0 SlowdownWritesTrigger，
      // 释放锁，sleep一秒来降低写入速度，给level 0的compact留时间
      mutex_.Unlock();
