...
private:
  ...
  // files_[i]中的FileMeta按照FileMetaData::smallest排序
...
private:
  ...
  // db元信息相关
  uint64_t next_file_number_;     // sstable文件编号，在db中全局递增
...
private:
  ...
  // 保存 grandparents_是因为 compact 最终会生成一系列 level-n+1 的 sstable, // 而如果生成的 sstable 与 level-n+2 中有过多的 overlap 的话,当 compact
  // level-n+1 时,会产生过多的 merge,为了尽量避免这种情况,compact 过程中 // 需要检查与 level-n+2 中产生 overlap 的 size 并与
